...
namespace UnityGameFramework.Editor.UI
{
    public class MenuOptionOverwrite
    {
        ...
        public static void AddInputField(MenuCommand menuCommand)
        {
            ...
            PlaceUIElementRoot(go, menuCommand);
            AttachAttribute<InputFieldT>(go);
        }
        ...
        public static void AttachAttribute<T>(GameObject go)
        {
            var types = GetTypes(System.AppDomain.CurrentDomain);
            foreach(var each in types)
            {
                System.Attribute[] attrs = System.Attribute.GetCustomAttributes(each);
                foreach (System.Attribute attr in attrs)
                {
                    if (attr is ExtForUIAttribute)
                    {
                        ExtForUIAttribute a = (ExtForUIAttribute)attr;
                        if(a.GetTargetType() == typeof(T))
                        {
                            go.AddComponent(each);
                        }
                    }
                }  
            }
        ...
        private static List<Type> GetTypes(System.AppDomain appDomain)
        {
